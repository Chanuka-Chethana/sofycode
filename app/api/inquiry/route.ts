...
export async function POST(request: Request) {
  try {
    ...
    if (name.trim().length < 2 || name.trim().length > 100) {
      return NextResponse.json(
        { message: 'Name must be between 2 and 100 characters. Example: "John Doe"' },
        { status: 400 }
      );
    }
    ...
    if (message.trim().length < 10) {
      return NextResponse.json(
        { message: 'Message is too short. Please provide at least 10 characters to describe your requirements.' },
        { status: 400 }
      );
    }
    ...
    if (message.trim().length > 5000) {
      return NextResponse.json(
        { message: 'Message is too long. Please limit it to 5000 characters.' },
        { status: 400 }
      );
    }
    ...
    try {
      ...
      return NextResponse.json(
        { message: 'Technical error: Could not send email. The team has been notified.' },
        { status: 500 }
      );
    }
